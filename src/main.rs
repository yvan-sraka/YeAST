...
fn exec(cmd: String, input: String) -> String {
    ...
    std::fs::create_dir_all("/tmp/yeast/").expect(&err("couldn't create the /tmp/yeast/ folder"));
    ...
    let output = std::process::Command::new("sh")
        .args(&["-c", &sh])
        .output()
        .expect(&err(&format!("failed to execute process: {}", sh)));
    ...
    loop {
            ...
            || buffer.contains("#!")
        {
            ...
            for child in vec {
                ...
                outputs += &child
                    .expect(&err("can't unwrap named thread"))
                    .join()
                    .unwrap(); // PANIC
            }
